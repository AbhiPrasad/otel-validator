...
export const int64Schema = z.union([
  ...
  message: 'Must be a string or number (64-bit integer). Large values should use string encoding to preserve precision.'
});
...
export const traceIdSchema = z.string({
  ...
  message: 'traceId must be exactly 32 hex characters (e.g., "5B8EFFF798038103D269B633813FC60C")'
});

/** Hex-encoded spanId (16 chars, case-insensitive) */
export const spanIdSchema = z.string({
  message: 'spanId must be a string'
}).regex(/^[a-fA-F0-9]{16}$/, {
  message: 'spanId must be exactly 16 hex characters (e.g., "EEE19B7EC3C1B174")'
});
