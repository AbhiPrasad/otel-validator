...
const spanSchema = z.object({
  ...
  name: z.string().optional(),
  kind: z.number({
    message: 'kind must be a number (integer). Use: UNSPECIFIED=0, INTERNAL=1, SERVER=2, CLIENT=3, PRODUCER=4, CONSUMER=5'
  }).int({
    message: 'kind must be an integer (0-5)'
  }).min(0, {
    message: 'kind must be >= 0'
  }).max(5, {
    message: 'kind must be <= 5 (UNSPECIFIED=0, INTERNAL=1, SERVER=2, CLIENT=3, PRODUCER=4, CONSUMER=5)'
  }).optional(),
